...
func (s *Session) FetchLogs(filter *LogFilter) ([]LogEntry, error) {
	...
	args := []string{"logs", "--machine"}
	cmd := exec.Command("flutter", args...)
	cmd.Dir = projectDir
	output, err := cmd.Output()
	if err != nil {
		return nil, fmt.Errorf("failed to fetch flutter logs: %w", err)
	}
...
func (s *Session) StreamLogs(stopChan chan struct{}) (chan LogEntry, error) {
	...
	go func() {
		...
		args := []string{"logs", "--machine", "--follow"}
		cmd := exec.Command("flutter", args...)
		cmd.Dir = projectDir
...
func (s *Session) SaveLogs(logs []LogEntry, outputDir string) (string, error) {
	...
	filename := fmt.Sprintf("flutter_logs_%s.txt", s.Timestamp())
